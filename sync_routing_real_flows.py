    ...
    if k == (mb1t,mb2t,mb3t):
      ...
      for km,vm in MBList.items():
        if vm[2] == mb1t:
          ...
          mb3.append(km)
      for kh,vh in AllHosts.items():
	#print v[2]
	#print vmMigCost(vm_s, kh)
        #print v[2][(str(kh)+'_i',str(0))],int(vmMigCost(vm_s, kh))
        v[2][(str(kh)+'_i',str(0))] = int(v[2][(str(kh)+'_i',str(0))])+int(vmMigCost(vm_s, kh))
	v[2][(str(kh)+'_f',str(1))] = int(v[2][(str(kh)+'_f',str(1))])+int(vmMigCost(vm_d, kh))
	v[2][(str(0), str(kh)+'_i')] = v[2][(str(kh)+'_i',str(0))]
	v[2][(str(1), str(kh)+'_f')] = v[2][(str(kh)+'_f',str(1))]

      for kh,vh in AllHosts.items():
        for mb1_ in mb1:
	  v[2][(str(kh)+'_i',mb1_)] = int(v[2][(str(kh)+'_i',mb1_)])*int(fr)
	  v[2][(mb1_, str(kh)+'_i')] = int(v[2][(mb1_, str(kh)+'_i')])*int(fr)
      for mb1_ in mb1:
        for mb2_ in mb2:
          v[2][(mb1_,mb2_)] = int(v[2][(mb1_,mb2_)])*int(fr)
          v[2][(mb2_,mb1_)] = int(v[2][(mb2_,mb1_)])*int(fr)
      for mb2_ in mb2:
        for mb3_ in mb3:
          v[2][(mb2_,mb3_)] = int(v[2][(mb2_,mb3_)])*int(fr)
          v[2][(mb3_,mb2_)] = int(v[2][(mb3_,mb2_)])*int(fr)
      for mb3_ in mb3:
        for kh,vh in AllHosts.items():	
	  v[2][(str(kh)+'_f',mb3_)] = int(v[2][(str(kh)+'_f',mb3_)])*int(fr)
	  v[2][(mb3_,str(kh)+'_f')] = int(v[2][(mb3_,str(kh)+'_f')])*int(fr)
...
def calcTotCommCost(vm, s):
  ...
  if len(vmFlows)>0:
    for f in vmFlows:
      if f[1] == vm:
        o = int(o) + int(f[1]) * int(shortest_distance(s,policies[f[4]][0],0))
      elif f[2] == vm:
        i = int(i) + int(f[1]) * int(shortest_distance(s,policies[f[4]][2],0))
...
def runSync():
  ...
  while True:
	...
	print 'start_exp=True'
      ...
      f_name = 'vmstat_sync.'+str(len(flows)/2)+'_'+str(len(policies)/2)+'.dat'
      f_name_stat_phasei = 'time_sync_pi.'+str(len(flows)/2)+'_'+str(len(policies)/2)+'.dat'
      f_name_stat_phaseii = 'time_sync_pii.'+str(len(flows)/2)+'_'+str(len(policies)/2)+'.dat'
      #f_name_stat_CV = 'time_sync_cv.'+str(len(flows)/2)+'_'+str(len(policies)/2)+'.dat'
	...
	start_time = time.time()
        phase_I(gr)
	end_time = time.time() - start_time
	f_msg = 'phase_I,%s,%s,%s' % (i,len(gr),end_time)
	print f_msg
	f_pi.write(f_msg+"\n")
...
class ProjectController(app_manager.RyuApp):
	...
	#nvm = 10000#50000#2000000#5000#10000#100#70 for test
        #np = 10#50
        nf = 10000#100000###3000#50#7 for test
	nvm = nf//10
	nvpt = 50 # -+25% the approximately number of vm per tenant so size of the group
